...
export default function ManagerDashboard({ user }) {
  ...
  const getOverdueTasks = (tasksList) => {
      ...
      .flatMap(task => {
          let assignedList = [];
          try { assignedList = JSON.parse(task.assigned_to) || []; } catch(e){}
  ...
  const fetchStats = () => {
    const url = user?.role === 'manager' ? `/manager/stats?managerId=${user.id}` : '/manager/stats';
    ...
    { 
      ...
      render: (_, r) => (
         <div style={{ display: 'flex', gap: 8 }}>
            <Button type="text" style={{ color: '#0ea5e9', background: '#f0f9ff', borderRadius: 8, padding: '4px 12px' }} icon={<EyeOutlined />} onClick={() => handleViewDetail(r)} />
            {/* Giả lập có pending task nếu thiếu giờ để test UI Quick Approve */}
            { parseFloat(r.total_hours || 0) === 0 && (
                <Button type="text" style={{ color: '#10b981', background: '#dcfce7', borderRadius: 8, padding: '4px 12px' }} icon={<CheckCircleOutlined />} onClick={() => message.success('Đã duyệt nhanh!')} />
            )}
         </div>
      ) 
    }
  ...
  const detailColumns = [
    ...
    { title: 'Hạng & Phản hồi', render: (_, record) => (
        <div style={{fontSize: 13}}>
            ...
            {record.feedback && <div style={{marginTop: 4, color: '#64748b'}}><i>"{record.feedback}"</i></div>}
        </div>
    )},
  ...
  const handleBulkApprove = () => {
      const promises = selectedRowKeys.map(id => axios.put('/work-logs/update-status', { id, status: 'Approved' }));
      Promise.all(promises).then(() => {
          message.success(`Đã duyệt ${selectedRowKeys.length} task!`);
          setSelectedRowKeys([]);
      ...
      },
      {
         ...
         render: (_, r) => (
            <div style={{ display: 'flex', gap: 8 }}>
                {r.status === 'Pending' ? (
                    <>
                        <Tooltip title="Duyệt"><Button type="text" style={{ color: '#10b981', background: '#dcfce7', borderRadius: 8 }} icon={<CheckCircleOutlined />} onClick={(e) => { e.stopPropagation(); handleSingleApprove(r.id); }} /></Tooltip>
                        <Tooltip title="Từ chối"><Button type="text" style={{ color: '#dc2626', background: '#fee2e2', borderRadius: 8 }} icon={<CloseOutlined />} onClick={(e) => { e.stopPropagation(); openRejectModal(r.id); }} /></Tooltip>
                    </>
                ) : (
                    <>
                        <Tooltip title="Cho phép sửa (về Draft)"><Button type="text" style={{ color: '#9333ea', background: '#faf5ff', borderRadius: 8 }} icon={<EditOutlined />} onClick={(e) => { e.stopPropagation(); handleAllowEdit(r.id); }} /></Tooltip>
                        <Tooltip title="Từ chối sửa (giữ nguyên Approved)"><Button type="text" style={{ color: '#dc2626', background: '#fee2e2', borderRadius: 8 }} icon={<CloseOutlined />} onClick={(e) => { e.stopPropagation(); handleRejectEdit(r.id); }} /></Tooltip>
                    </>
                )}
            </div>
         )
    ...
    <div>
               ...
               <Card style={{ borderRadius: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.02)', border: '1px solid #f1f5f9' }} bodyStyle={{ padding: '20px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                     <div>
                        ...
                        <div style={{ fontSize: 12, color: '#10b981', fontWeight: 500 }}>↑ +1 so với tháng trước</div>
                     </div>
      ...
      >
        <div style={{ padding: '16px 24px', background: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
            ...
            <Button type="primary" style={{ background: '#10b981', borderRadius: 8, fontWeight: 500, boxShadow: '0 4px 12px rgba(16, 185, 129, 0.2)' }} disabled={selectedRowKeys.length === 0} onClick={handleBulkApprove}>Duyệt tất cả</Button>
        </div>
